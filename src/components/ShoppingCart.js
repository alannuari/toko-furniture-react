        ...
        <div className="w-full px-4 mb-4 md:w-8/12 md:mb-0" id="shopping-cart">
            ...
            {Object.keys(state.cart).length === 0 ? (
                <p id="cart-empty" className="text-center py-8">
                    Ooops... Cart is empty.{' '}
                    <Link to="/" className="underline text-pink-400 font-bold">
                        Shop Now
                    </Link>
                </p>
            ) : (
                Object.keys(state.cart).map((key, idx) => {
                    ...
                    return (
                        <div key={key} className="flex flex-start flex-wrap items-center mb-4 -mx-4" data-row={idx}>
                            ...
                            <div className="px-4 w-2/12">
                                <div className="text-center">
                                    <button onClick={() => dispatch({ type: 'REMOVE_FROM_CART', id: item.id })} className="text-red-600 border-none focus:outline-none px-3 py-1">
                                        X
                                    </button>
                                </div>
                            </div>
